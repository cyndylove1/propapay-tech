...
import {getFromLocalStorage, removeFromLocalStorage, saveToLocalStorage} from '@/utils/storageUtils';
...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const verifyEmailMutation = useMutation({
    ...
    onError: (error: unknown) => {
      const errorMessage = error instanceof Error 
        ? error.message 
        : (error as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Email verification failed. Please try again.';
      setAuthState(prev => ({ ...prev, error: errorMessage }));
      toast.error(errorMessage);
    },
  });
  ...
  const forgotPasswordMutation = useMutation({
    ...
    onError: (error: unknown) => {
      const errorMessage = error instanceof Error 
        ? error.message 
        : (error as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Failed to process request. Please try again.';
      setAuthState(prev => ({ ...prev, error: errorMessage }));
      toast.error(errorMessage);
    },
  });
  ...
  const resetPasswordMutation = useMutation({
    ...
    onError: (error: unknown) => {
      const errorMessage = error instanceof Error 
        ? error.message 
        : (error as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Password reset failed. Please try again.';
      setAuthState(prev => ({ ...prev, error: errorMessage }));
      toast.error(errorMessage);
    },
  });
  ...
  const updateProfileMutation = useMutation({
    ...
    onError: (error: unknown) => {
      const errorMessage = error instanceof Error 
        ? error.message 
        : (error as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Profile update failed. Please try again.';
      setAuthState(prev => ({ ...prev, error: errorMessage }));
      toast.error(errorMessage);
    },
  });
  ...
  const changePasswordMutation = useMutation({
    ...
    onError: (error: unknown) => {
      const errorMessage = error instanceof Error 
        ? error.message 
        : (error as { response?: { data?: { message?: string } } })?.response?.data?.message || 'Password change failed. Please try again.';
      setAuthState(prev => ({ ...prev, error: errorMessage }));
      toast.error(errorMessage);
    },
  });
